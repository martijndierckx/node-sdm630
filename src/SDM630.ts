...
export class SDM630 {
   ...
   */
  public static async connect(opts: SDM630Config): Promise<SDM630> {
    const sdm630 = new SDM630();

    // Connect to modbus
    sdm630.modbusConn = await ModbusConnection.connect({
      host: opts.host,
      port: opts.port ?? 502,
   ...
   */
  public async getValues(): Promise<SDM630Registers> {
